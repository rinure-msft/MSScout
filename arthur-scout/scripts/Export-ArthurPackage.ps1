    ...
    "steps": [
      {
        "id": "1",
        "label": "Main",
        "prompt": "Run `python \"<SCRATCHPAD_PATH>\\arthur_scout_handoff.py\" --next`. It prints one JSON object. Always respond visibly; never stay quiet. If status is `no_pending`, respond exactly: `No Arthur Scout handoff pending.` If status is `pending` and type is `action_tracker`, execute the returned `source_prompt` using Scout tools. Use Azure DevOps project `https://dev.azure.com/FraudOps/Fraud%20Ops%20AI%20Tracker`, create/update distinct work items tagged `ArthurActionTracker`, assign to Rin Ure unless explicitly different, and send an email only to Rin.Ure@microsoft.com with the ADO links. Do not send Teams messages for Action Tracker handoffs. Close Playwright if used. After successful execution, run `python \"<SCRATCHPAD_PATH>\\arthur_scout_handoff.py\" --mark-done \"<returned id>\" --response \"<response_after_completion or Done.>\"` and respond with that response. If execution fails, run `python \"<SCRATCHPAD_PATH>\\arthur_scout_handoff.py\" --mark-failed \"<returned id>\" --reason \"<failure reason>\"` and report the failure."
      }
    ]
  },
  {
