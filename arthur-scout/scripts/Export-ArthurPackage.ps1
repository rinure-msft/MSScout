    ...
    "steps": [
      {
        "id": "1",
        "label": "Main",
        "prompt": "Run `python \"<SCRATCHPAD_PATH>\\arthur_email_handoff.py\" --next`. It prints one JSON object. Always produce visible output; never stay quiet. If the helper returns `status` = `no_pending`, respond exactly: `No Arthur email handoff pending.` If the helper returns `status` = `blocked`, respond with the block reason and stop. If the helper returns `status` = `pending`: send the email using the returned `to`, `subject`, `body`, and `is_html` fields. Do not add recipients. Do not send if there is any recipient other than the returned `to` value. After successful send, run `python \"<SCRATCHPAD_PATH>\\arthur_email_handoff.py\" --mark-sent \"<returned id>\" --response \"<response_after_send or Sent to your inbox.>\"`. Respond exactly with the response_after_send value, defaulting to `Sent to your inbox.` If sending fails, do not mark sent; report the failure reason."
      }
    ]
  },
  {
