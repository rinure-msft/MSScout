...
1. Voice bridge: records microphone audio, transcribes continuous activation and short commands with Zipformer, matches enabled local voice commands, speaks responses and queues larger Scout/Copilot tasks.
...
Zipformer is the only production recognizer. It handles continuous activation,
inline commands and the next utterance after a wake-only activation. Arthur
requires the configured wake name at the start of an utterance, optionally
after a short greeting such as `hey` or `okay`, and does not expose commands
omitted from `enabledCommands`.
